...
;;; The Linux/PPC 32-bit ABI says:
;;;
;;;   If a single-float arg has to go on the stack, it's promoted to a double.
;;;
;;; gcc does:
;;;
...
;;; If a single-float arg has to go on the stack, it's promoted to double. That way,
;;; C programs can get subtle rounding errors when unrelated arguments are introduced.
#+darwin
(define-alien-type-method (single-float :arg-tn) (type state)
  (declare (ignore type))
  (let* ((fprs (arg-state-fpr-args state))
...
(define-alien-type-method (double-float :arg-tn) (type state)
  ...
  (let ((fprs (arg-state-fpr-args state))
    ...
    (cond ((< gprs 8) ; and by implication also (< fprs 13)
           ...
           ;; to %alien-funcall ir2-convert by making a list of
           ;; the TNs for the float reg and for the int regs.
           ;;
...
(progn
  ...
  ;;; On Linux only use 8 bytes for LR and Back chain.
  ;;; JRXR. 2006/11/10.
  #-darwin
  (defconstant n-foreign-linkage-area-bytes 8)

  ;;; Returns a vector in static space containing machine code
  ;;; for the callback wrapper. Linux version. JRXR. 2006/11/13
  #-darwin
  ...
  ;;; Returns a vector in static space containing machine code for the callback wrapper.
  #+darwin
  (defun alien-callback-assembler-wrapper (index result-type argument-types)
    (flet ((make-gpr (n)
             (make-random-tn :kind :normal :sc (sc-or-lose 'any-reg) :offset n))
      ...
      (let* ((segment (make-segment)))
        (assemble (segment)
          ;; To save our arguments, we follow the algorithm sketched
          ;; in the "PowerPC Calling Conventions" section of that document.
          ;;
          ...
          (let* ((n-return-area-words
            ...
            (destructuring-bind (sp r0 arg1 arg2 arg3 arg4)
              ...
              (flet ((load-address-into (reg addr)
                ...
                ;; FIXME: What does this FIXME refer to?
